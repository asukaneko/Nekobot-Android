...
abstract class NekobotDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * v14 → v15：local_sessions 新增 plot_outline 列（剧情大纲文本，AI 生成选项时围绕此走向推进）。
         */
        private val MIGRATION_14_15 = object : Migration(14, 15) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_sessions ADD COLUMN plot_outline TEXT")
            }
        }
        ...
        /**
         * v16 → v17：local_state_snapshots 新增 user_message/assistant_message 列
         *（本轮对话原文，供「状态历程」界面底部对话回放展示）。
         */
        private val MIGRATION_16_17 = object : Migration(16, 17) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_state_snapshots ADD COLUMN user_message TEXT")
                db.execSQL("ALTER TABLE local_state_snapshots ADD COLUMN assistant_message TEXT")
            }
        }
        ...
        /** v22 → v23：持久化 Agent 助手消息的完整工具调用历史。 */
        private val MIGRATION_22_23 = object : Migration(22, 23) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_messages ADD COLUMN tool_call_history TEXT")
            }
        }

        /** v23 → v24：为每个本地 AI 模型增加独立代理链接。 */
        private val MIGRATION_23_24 = object : Migration(23, 24) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_ai_models ADD COLUMN proxy_url TEXT NOT NULL DEFAULT ''")
            }
        }

        /**
         * v24 → v25：补齐本地自动化状态与主动聊天消息来源。
         *
         * 所有变更均为追加列，升级时保留现有会话、任务和工作流配置。
         */
        val MIGRATION_24_25 = object : Migration(24, 25) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_messages ADD COLUMN source TEXT")
                db.execSQL("ALTER TABLE local_tasks ADD COLUMN status TEXT NOT NULL DEFAULT 'idle'")
                db.execSQL("ALTER TABLE local_tasks ADD COLUMN last_error TEXT")
                db.execSQL("ALTER TABLE local_workflows ADD COLUMN session_id TEXT")
                db.execSQL("ALTER TABLE local_workflows ADD COLUMN last_run TEXT")
                db.execSQL("ALTER TABLE local_workflows ADD COLUMN next_run TEXT")
                db.execSQL("ALTER TABLE local_workflows ADD COLUMN status TEXT NOT NULL DEFAULT 'idle'")
                db.execSQL("ALTER TABLE local_workflows ADD COLUMN last_error TEXT")
            }
        }

        /** v25 → v26：增加本地知识库文档与切片索引表。 */
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL(
                    ...
                    CREATE TABLE IF NOT EXISTS local_knowledge_chunks (
                        ...
                        content TEXT NOT NULL,
                        embedding_json TEXT,
                        FOREIGN KEY(document_id) REFERENCES local_knowledge_documents(id) ON UPDATE NO ACTION ON DELETE CASCADE
                    )
                    """.trimIndent()
                )
                db.execSQL("CREATE INDEX IF NOT EXISTS index_local_knowledge_chunks_document_id ON local_knowledge_chunks(document_id)")
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_local_knowledge_chunks_document_id_chunk_index ON local_knowledge_chunks(document_id, chunk_index)")
            }
        }
        ...
        /** v27 → v28：本地 HTTP MCP 服务增加自定义请求头。 */
        val MIGRATION_27_28 = object : Migration(27, 28) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_mcp_servers ADD COLUMN headers_json TEXT")
            }
        }

        /** v28 → v29：助手消息保存模型思考/推理内容。 */
        val MIGRATION_28_29 = object : Migration(28, 29) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_messages ADD COLUMN reasoning_content TEXT")
            }
        }
        ...
        /** v35 → v36：为对话模型增加视觉输入能力开关。 */
        val MIGRATION_35_36 = object : Migration(35, 36) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE local_ai_models ADD COLUMN supports_vision INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
        ...
        /** v37 → v38：Agent 会话目标（/goal）与规格任务（/spec），会话级持久化。 */
        val MIGRATION_37_38 = object : Migration(37, 38) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE local_sessions ADD COLUMN agent_goal TEXT")
                db.execSQL("ALTER TABLE local_sessions ADD COLUMN agent_spec TEXT")
            }
        }
