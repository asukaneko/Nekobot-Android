...
internal class LocalPluginTool(
    ...
    private companion object {
        ...
        val PLUGIN_DEV_GUIDE: String = """
            ...
            - pages: 页面数组（≤8 个），每项 {"id": "小写id", "title": "标题", "title_i18n": {...}, "entry": "pages/x.html", "styles": [...], "scripts": [...], "order": 100}；entry 必须是以 .html 结尾的安全相对路径且文件真实存在。页面用 host.* API（host.storage.*、host.chat.*（含 chat.messages.append/send、chat.sessions.create/switch、chat.context/sessionConfig/promptStack/toolCalls）、host.characters.*（含 create/update）、host.worldbooks.*、host.memory.read/write/append/edit、host.ui.render、host.http.get、host.ui.toast、host.system.info）；页面内可用相对链接或 host.ui.openPage(pageId, args) 在同插件目录的多个 HTML 之间切换（返回键逐页回退，外部跳转仍被拦截）；原生弹窗：window.alert/confirm/prompt 与 host.ui.alert/confirm/prompt/select（select 传 {options: [...]}，取消返回 null），与 ctx.api 共用权限与存储。
