...
(function () {
  ...
  const FALLBACK_RELEASES = [
    { tag_name: 'v0.7.7', published_at: '2026-09-25', body: '- 本地 MCP 支持在 Alpine 沙盒中运行 stdio 服务，缺运行时会提示安装\n- Skill 支持 AI 通过数据库工具直接写入 SKILL.md 与附加文件\n- read_image 支持视觉对话模型直接注入图片，进度卡片净化 base64\n- Agent 循环中间回复留档进度卡片，聊天页内联完整展示\n- 插件新增 workspace 权限，可读写插件专属工作区文件\n- 上下文压缩按钮显示「压缩中」进度状态\n- 修复上下文压缩超时中断、插件小组件点击无响应等问题\n- 修复低版本 Android 崩溃与 Skill 导出 ZIP 失败\n- 相邻思考卡片合并为整组圆角卡片\n- 完善代币来源与用途的本地化文案' },
    { tag_name: 'v0.7.6', published_at: '2026-09-21', body: '- 插件页面：插件可在应用内提供自己的页面，跟随主题就地换肤，支持多页面切换\n- 插件新增 12 项权限白名单，按「清单声明 ∧ 用户授权」生效，可随时撤销\n- 插件宿主 API：读写会话 / 角色 / 世界书 / 记忆、联网、发消息、调用 AI\n- 插件新增会话洞察 API 与 chat.progress 进度卡片能力\n- 插件卡片展示页面数、入口、权限状态、钩子与兼容级别，新增小组件入口\n- Agent 会话支持「继承完整角色能力」，可加载角色卡 / 记忆 / 世界书\n- 新增 Agent 等待处理提醒中心，等待项在会话列表标记并推送通知\n- 子代理新增 outputting 状态，避免父 Agent 长时间空等\n- 进度卡片与「思考中」气泡融合，工具参数改为「参数名 → 参数值」展示\n- Tokens 新增「本周」范围与 CSV 导出，排行榜按当前时间范围聚合\n- 修复 Agent 流式思考重复累积与停更、Token 价格显示等问题' },
    { tag_name: 'v0.7.5', published_at: '2026-09-19', body: '- 安全加固：令牌仅在服务器主机附加，外链图片不再能窃取用户令牌\n- 安全加固：自更新校验 APK 包名与签名，无障碍新增排除应用与不可记忆确认\n- 安全加固：远程配置导入防 ZIP 路径穿越并限制包体与解压总量\n- 安全加固：插件与沙盒终端受网络访问开关约束，禁止访问内网与本机地址\n- 安全加固：解释器类命令不再支持「始终允许」，预览 WebView 统一下沉加固\n- Agent 记忆新增记忆间隔配置，内联提示锚定到触发回复下方\n- 自动长期记忆改为「先审查再改写」，支持单条新增 / 改写 / 删除\n- 进度卡按轮次拆分思考步骤，详情弹窗支持流式跟随\n- 本地 AI 模型支持启用 / 停用，停用后不再进入故障转移队列\n- Skill 支持导出为 ZIP，工具详情展示说明与输入参数' },
  ];
