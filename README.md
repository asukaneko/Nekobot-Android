...
- **💬 沉浸式对话** — Socket.IO 流式回复、乐观更新、重新生成 / 停止生成、编辑并重新发送、生成期间消息排队与加急发送、消息分叉与多选操作
...
- **🧠 记忆管理** — 角色记忆的查看与编辑
- **🤖 Agent 模式** — 多轮工具调用、可展开的实时进度卡片、子代理委派、任务列表与结构化问答、原生浏览器、Android 设备操作、Linux 沙箱、会话终端与文件处理
...
- **🗂 工作区** — 文件引用、预览与下载
- **🧩 插件系统** — 本地模式支持内置模块与 ZIP JavaScript 插件，命令、会话读取、隔离存储、通知和受控 HTTPS 请求均由显式权限管理，Agent 可通过 `plugin_use` 工具安装、启停与卸载插件；开发方式见[插件开发指南](docs/plugin-development.md)
...
| 能力               | 说明                                                     |
| ---------------- | ------------------------------------------------------ |
| **多轮工具执行**       | 模型可连续规划和调用工具；进度卡片实时显示思考、参数、结果、错误与执行耗时，用户展开状态不会因新工具事件而重置     |
| **子代理**          | 独立任务可委派给拥有独立上下文的子代理，支持前台同步与后台异步运行、受限嵌套委派；进度以独立卡片实时推送，并提供启用开关与嵌套深度 / 工具上限设置 |
| **会话工具集**        | 工具按大类归组，可在聊天面板按大类或单工具开关当前会话的工具集并按会话持久化，默认全部启用，支持一键重置 |
...
| **会话终端**         | Agent 会话右上角菜单可打开全屏命令行，直接操作该会话的 `/workspace`；配套沙箱文件浏览器支持目录浏览、≤16KB 文本文件编辑保存与大文件只读预览            |
...
| **运行保护**         | 高风险命令需要确认；工具参数会在执行前修复和校验，连续重复且无进展的调用会自动停止，最大工具调用次数可配置（默认 150）              |
...
| **Agent** | 由服务器能力决定                | 内置浏览器、Linux 沙箱、终端、Skills、MCP、子代理与 Android 操作     |
...
6. 在聊天面板的「Agent 工具集」中可按大类或单个工具开关本会话可用工具，自定义选择按会话持久化
